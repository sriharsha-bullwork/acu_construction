document.addEventListener('DOMContentLoaded', () => {
    ...
    const gpsRefWarn = document.getElementById('gps-ref-warning');
    ...
    // --- State ---
    const defaultSettings = { proximity: 0.5, recordDensity: 0.2 };
    let localRouteData = { waypoints: [], routes: {}, settings: { ...defaultSettings } };
    let nav2Path = [];
    let recordedPath = [];
    let pose = { x: 0, y: 0, yaw: 0, yaw_deg: 0 };
    let poseGPS = { lat: 0, lon: 0, heading_deg: 0 };
    let gpsRef = { lat0: null, lon0: null, m_per_deg_lat: 111320, m_per_deg_lon: 111320 };
    ...
    function latlonToXY(lat, lon) {
        ...
        const dx = (lon - gpsRef.lon0) * gpsRef.m_per_deg_lon;
        const dy = (lat - gpsRef.lat0) * gpsRef.m_per_deg_lat;
        return { x: dx, y: dy };
    }
    ...
    function cardFromHeading(deg){
        ...
        const ix = Math.round(((deg % 360) / 360) * dirs.length) % dirs.length; return dirs[ix];
    }

    function drawArrowGPS(lat,lon,headingDeg,color='#d73a49',size=12,isNear=false){
        const p = worldToScreenLatLon(lat, lon);
        if(isNear){ctx.fillStyle='rgba(40,167,69,0.3)';ctx.beginPath();ctx.arc(p.x,p.y,15,0,2*Math.PI);ctx.fill();}
        ctx.save();ctx.translate(p.x,p.y);ctx.rotate(-headingDeg*Math.PI/180);
        ctx.fillStyle=color;ctx.beginPath();ctx.moveTo(size,0);ctx.lineTo(-size*0.6,size*0.6);ctx.lineTo(-size*0.6,-size*0.6);ctx.closePath();ctx.fill();ctx.restore();
    }
    function drawPathGPS(path,color,dashed=false,width=3){
        if(!path || path.length<2)return;ctx.strokeStyle=color;ctx.lineWidth=width;if(dashed)ctx.setLineDash([8,8]);
        ctx.beginPath();
        const p0 = worldToScreenLatLon(path[0].lat, path[0].lon); ctx.moveTo(p0.x,p0.y);
        for(const point of path){ const p = worldToScreenLatLon(point.lat, point.lon); ctx.lineTo(p.x,p.y); }
    ...
    function render() {
        ...
        const proximity = (localRouteData.settings && typeof localRouteData.settings.proximity === 'number') ? localRouteData.settings.proximity : defaultSettings.proximity;
        const proximitySq = proximity * proximity;
        localRouteData.waypoints.forEach((wp, index) => {
            const a = latlonToXY(poseGPS.lat, poseGPS.lon);
            const b = latlonToXY(wp.lat, wp.lon);
            const distSq = (a.x - b.x)**2 + (a.y - b.y)**2;
            const isNear = distSq < proximitySq;
            drawArrowGPS(wp.lat, wp.lon, wp.yaw_deg || 0, '#ff7b2b', 8, isNear);
            const p = worldToScreenLatLon(wp.lat, wp.lon);
    ...
    function updateUI(data) {
        ...
        poseGPS = data.pose_gps || poseGPS;
        nav2Path = data.nav2_path || [];
        recordedPath = data.recorded_path || [];
        isRecording = data.is_recording;
        gpsRef = data.gps_ref || gpsRef;
        const hasGpsRef = gpsRef && gpsRef.lat0 != null && gpsRef.lon0 != null;
        if (gpsRefWarn) gpsRefWarn.classList.toggle('hidden', !!hasGpsRef);
        ...
        if (data.mission_mode === 'idle') {
            ...
            if (isRecording && recordingControls) {
                ...
                btnStartRecording.disabled = true; btnFinishRecording.disabled = false;
            } else if (recordingControls) {
                if (!recordingControls.dataset.sticky) recordingControls.classList.add('hidden');
                btnStartRecording.disabled = false; btnFinishRecording.disabled = true;
            }
        ...
        gpsEl.textContent = `GPS: lat=${poseGPS.lat.toFixed(6)}, lon=${poseGPS.lon.toFixed(6)}, hdg=${poseGPS.heading_deg.toFixed(1)}° (${cardFromHeading(poseGPS.heading_deg)})`;
    ...
    function renderRecordedPoints() {
        ...
        let html = '<table><thead><tr><th>#</th><th>Lat</th><th>Lon</th><th>Heading°</th></tr></thead><tbody>';
        recordedPath.forEach((p, i) => {
            const yaw = (typeof p.yaw_deg === 'number') ? p.yaw_deg : (p.yaw ? (p.yaw * 180/Math.PI) : 0);
            html += `<tr><td>${i}</td><td>${Number(p.lat).toFixed(6)}</td><td>${Number(p.lon).toFixed(6)}</td><td>${Number(yaw).toFixed(1)}</td></tr>`;
        });
        html += '</tbody></table>';
    ...
    function renderTables() {
        let wpHtml = '<table><thead><tr><th>Name</th><th>Lat</th><th>Lon</th><th>Heading°</th><th></th></tr></thead><tbody>';
        localRouteData.waypoints.forEach((wp, i) => {
            wpHtml += `<tr>
                <td><input type="text" class="wp-input" data-index="${i}" data-field="name" value="${wp.name || ''}"></td>
                <td><input type="number" step="0.000001" class="wp-input" data-index="${i}" data-field="lat" value="${Number(wp.lat).toFixed(6)}"></td>
                <td><input type="number" step="0.000001" class="wp-input" data-index="${i}" data-field="lon" value="${Number(wp.lon).toFixed(6)}"></td>
                <td><input type="number" class="wp-input" data-index="${i}" data-field="yaw_deg" value="${Number(wp.yaw_deg||0).toFixed(1)}"></td>
    ...
    btnAddWp.onclick = () => {
        localRouteData.waypoints.push({ id: `wp${Date.now()}`, name: '', lat: poseGPS.lat, lon: poseGPS.lon, yaw_deg: poseGPS.heading_deg });
        renderTables();
    ...
    btnAddRoute.onclick = () => {
        if (recordingControls) { recordingControls.classList.remove('hidden'); recordingControls.dataset.sticky = '1'; }
        renderWaypointSelects();
    };
    ...
    routeTableContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('btn-delete-route')) {
            ...
            renderTables();
            fetch('/api/set_route_data', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(localRouteData) });
        } else if (e.target.classList.contains('btn-record-route')) {
            const { from, to } = e.target.dataset;
            if (recordingControls) {
                recordingControls.classList.remove('hidden'); recordingControls.dataset.sticky = '1';
            }
            if (recordFromSelect && recordToSelect) {
                recordFromSelect.value = from; recordToSelect.value = to;
                recordFromSelect.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
        } else if (e.target.classList.contains('btn-edit-route')) {
            const key = e.target.dataset.key; showRouteEditor(key);
        }
    });
    ...
    btnStartRecording.onclick = async () => {
        const from = recordFromSelect?.value; const to = recordToSelect?.value;
        if (!from || !to) { alert('Select both From and To waypoints'); return; }
        if (!localRouteData.routes[`${from}-${to}`]) { localRouteData.routes[`${from}-${to}`] = []; renderTables(); }
        ...
        routeTableContainer.dataset.recordingTo = to; btnStartRecording.disabled = true; btnFinishRecording.disabled = false;
    };
    btnFinishRecording.onclick = async () => {
        const to = recordToSelect?.value || routeTableContainer.dataset.recordingTo; if (!to) { alert('No destination selected'); return; }
        await fetch('/api/stop_recording', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ to_wp_id: to })});
        btnStartRecording.disabled = false; btnFinishRecording.disabled = true; if (recordingControls) delete recordingControls.dataset.sticky;
    };
    ...
    function showRouteEditor(key) {
        ...
        if (recordingControls) { recordingControls.classList.remove('hidden'); recordingControls.dataset.sticky = '1'; const [from, to] = key.split('-'); if (recordFromSelect && recordToSelect) { recordFromSelect.value = from; recordToSelect.value = to; } }
    ...
    function renderRoutePointsEditor() {
        ...
        let html = '<table><thead><tr><th>#</th><th>Lat</th><th>Lon</th><th>Heading°</th><th></th></tr></thead><tbody>';
        path.forEach((p, i) => {
            const yaw = (typeof p.yaw_deg === 'number') ? p.yaw_deg : (p.yaw ? (p.yaw * 180/Math.PI) : 0);
            html += `<tr>
                <td>${i}</td>
    ...
    if (btnExport) {
        btnExport.onclick = () => {
            ...
            downloadJSON('routes_gps.json', data);
        };
    }
