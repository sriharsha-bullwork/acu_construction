...
class NavCommander(Node):
    def __init__(self):
        ...
        # Local pose (from odom, used only for UI drawing compatibility)
        self.pose = {'x': 0.0, 'y': 0.0, 'yaw': 0.0, 'yaw_deg': 0.0}
        # GPS pose (lat/lon/alt + heading from IMU if available)
        self.gps_pose = {'lat': None, 'lon': None, 'alt': None, 'yaw_deg': 0.0}
        self.nav2_path = []
        self.log_messages = deque(maxlen=100)
        # Keep local route data for UI compatibility
        self.route_data = {'waypoints': [], 'routes': {}, 'settings': {'proximity': 0.2, 'recordDensity': 0.1}}
        # Separate GPS route data (lat/lon)
        self.route_data_gps = {'waypoints': [], 'routes': {}, 'settings': {'proximity': 0.2, 'recordDensity': 0.1}}
        
        self.mission_mode = 'idle'
        self.is_moving = False
        self.is_paused = False
        self.is_recording = False
        # Local recorded path (unused in GPS mode, kept for UI)
        self.recorded_path = []
        # GPS recorded path: list of {'lat','lon','yaw_deg'[, 'id']}
        self.recorded_path_gps = []
        ...
        # GPS + IMU inputs (absolute frame)
        self.create_subscription(NavSatFix, '/gps/fix', self._gps_cb, qos_profile_sensor_data)
        self.create_subscription(Imu, 'imu_gps/data', self._imu_cb, qos_profile_sensor_data)
    ...
    def _odom_cb(self, msg: Odometry):
        ...
        # Local path recording disabled in GPS mode

    def _imu_cb(self, msg: Imu):
        o = msg.orientation
        _, _, yaw = euler_from_quaternion([o.x, o.y, o.z, o.w])
        self.gps_pose['yaw_deg'] = (math.degrees(yaw) + 360.0) % 360.0

    def _gps_cb(self, msg: NavSatFix):
        lat = float(msg.latitude)
        lon = float(msg.longitude)
        alt = float(msg.altitude)
        self.gps_pose.update({'lat': lat, 'lon': lon, 'alt': alt})
        # Drive GUI pose directly from GPS (keep in GPS reference: x=lon, y=lat)
        self.pose['x'] = lon
        self.pose['y'] = lat
        self.pose['yaw_deg'] = self.gps_pose.get('yaw_deg', 0.0)
        if self.is_recording and lat != 0.0 and lon != 0.0:
            last_pt = self.recorded_path_gps[-1] if self.recorded_path_gps else None
            if (last_pt is None) or (haversine_m(last_pt['lat'], last_pt['lon'], lat, lon) > float(self.route_data.get('settings', {}).get('recordDensity', 0.1))):
                self.recorded_path_gps.append({'lat': lat, 'lon': lon, 'yaw_deg': self.gps_pose['yaw_deg']})

    def set_route_data(self, data: Dict):
        # Decide where to store: GPS vs local, based on key presence
        ...
        # Heuristic: treat as GPS data if waypoints have 'lat'/'lon'
        is_gps = any(('lat' in w and 'lon' in w) for w in normalized['waypoints'])
        if is_gps:
            self.route_data_gps = normalized
            self.log_message(f'Updated GPS route data: {len(normalized.get("waypoints",[]))} waypoints, {len(normalized.get("routes",{}))} routes.')
        else:
            self.route_data = normalized
            self.log_message(f'Updated local route data: {len(normalized.get("waypoints",[]))} waypoints, {len(normalized.get("routes",{}))} routes.')

    def start_recording(self, from_wp_id):
        if self.mission_mode != 'idle': return
        self.is_recording = True
        self._record_from_id = from_wp_id
        wp = next((w for w in self.route_data_gps['waypoints'] if w['id'] == from_wp_id), None)
        if wp:
            # Start at the GPS waypoint with its orientation and keep the id for saving
            self.recorded_path_gps = [{'lat': float(wp.get('lat', 0.0)), 'lon': float(wp.get('lon', 0.0)), 'yaw_deg': float(wp.get('yaw_deg', 0.0)), 'id': from_wp_id}]
        else:
            # Start at current GPS pose, still remember the intended from id
            p = {'lat': self.gps_pose['lat'], 'lon': self.gps_pose['lon'], 'yaw_deg': self.gps_pose['yaw_deg'], 'id': from_wp_id}
            self.recorded_path_gps = [p]
        self.log_message(f'GPS route recording started from {from_wp_id}.')

    def stop_recording(self, to_wp_id):
        if not self.is_recording: return
        from_wp_id = self.recorded_path_gps[0].get('id') if self.recorded_path_gps else self._record_from_id
        if from_wp_id and to_wp_id:
            # Build final path, optionally interpolate straight to the destination waypoint
            to_wp = next((w for w in self.route_data_gps['waypoints'] if w['id'] == to_wp_id), None)
            path = list(self.recorded_path_gps)
            if to_wp:
                if not path:
                    path = [{'lat': float(to_wp.get('lat', 0.0)), 'lon': float(to_wp.get('lon', 0.0)), 'yaw_deg': float(to_wp.get('yaw_deg', 0.0))}]
                else:
                    last = path[-1]
                    dist = haversine_m(float(last['lat']), float(last['lon']), float(to_wp.get('lat', 0.0)), float(to_wp.get('lon', 0.0)))
                    ...
                    if dist > step * 0.5:
                        ...
                        for i in range(1, steps_n + 1):
                            t = i / steps_n
                            ilat = float(last['lat']) + (float(to_wp.get('lat', 0.0)) - float(last['lat'])) * t
                            ilon = float(last['lon']) + (float(to_wp.get('lon', 0.0)) - float(last['lon'])) * t
                            yaw_deg = bearing_deg(float(last['lat']), float(last['lon']), float(to_wp.get('lat', 0.0)), float(to_wp.get('lon', 0.0)))
                            path.append({'lat': ilat, 'lon': ilon, 'yaw_deg': yaw_deg})
            ...
            self.route_data_gps['routes'][route_key] = path
            # Also create the reverse route
            rev_key = f"{to_wp_id}-{from_wp_id}"
            rev_path = list(reversed([{ 'lat': p['lat'], 'lon': p['lon'], 'yaw_deg': (p.get('yaw_deg', 0.0) + 180.0) % 360.0 } for p in path]))
            # For reverse, set final yaw to from_wp's yaw if available
            from_wp = next((w for w in self.route_data_gps['waypoints'] if w['id'] == from_wp_id), None)
            ...
            self.route_data_gps['routes'][rev_key] = rev_path
            self.log_message(f'Route {route_key} saved with {len(path)} points. Reverse saved as {rev_key}.')
        self.is_recording = False; self.recorded_path_gps = []; self._record_from_id = None
    ...
    def _apply_headings_gps(self, path, final_yaw_deg: float = None):
        ...
        for i in range(len(path) - 1):
            p = path[i]
            n = path[i + 1]
            yaw_deg = bearing_deg(float(p['lat']), float(p['lon']), float(n['lat']), float(n['lon']))
            out.append({'lat': float(p['lat']), 'lon': float(p['lon']), 'yaw_deg': yaw_deg})
        last = path[-1]
        if len(path) >= 2 and final_yaw_deg is None:
            prev = path[-2]
            yaw_deg = bearing_deg(float(prev['lat']), float(prev['lon']), float(last['lat']), float(last['lon']))
...
def status():
    # Build a display-friendly view using GPS reference for the existing UI
    # Convert GPS routes to x/y as lon/lat for drawing; keep yaw_deg
    disp_routes = {}
    for key, path in node.route_data_gps.get('routes', {}).items():
        disp_routes[key] = [{'x': float(p.get('lon', 0.0)), 'y': float(p.get('lat', 0.0)), 'yaw_deg': float(p.get('yaw_deg', 0.0))} for p in path]
    disp_waypoints = []
    for w in node.route_data_gps.get('waypoints', []):
        disp_waypoints.append({
            'id': w.get('id'),
            'name': w.get('name', ''),
            'x': float(w.get('lon', 0.0)),
            'y': float(w.get('lat', 0.0)),
            'yaw_deg': float(w.get('yaw_deg', 0.0)),
        })
    disp_route_data = {
        'waypoints': disp_waypoints,
        'routes': disp_routes,
        'settings': node.route_data_gps.get('settings', node.route_data.get('settings', {})),
    }
    # Map recorded GPS path to x/y for UI table + drawing
    disp_recorded = [{'x': float(p.get('lon', 0.0)), 'y': float(p.get('lat', 0.0)), 'yaw_deg': float(p.get('yaw_deg', 0.0))} for p in node.recorded_path_gps]

    return jsonify({
        'pose': node.pose,                    # x=lon, y=lat
        'gps_pose': node.gps_pose,
        ...
        'route_data': disp_route_data,        # drive UI with GPS reference
        'route_data_gps': node.route_data_gps,
        'recorded_path': disp_recorded,       # UI expects x/y
        'recorded_path_gps': node.recorded_path_gps,
    })
...
@app.route('/api/export_route_data_gps', methods=['GET'])
def export_route_data_gps():
    from flask import Response
    import json
    data = json.dumps(node.route_data_gps, indent=2)
    resp = Response(data, mimetype='application/json')
    resp.headers['Content-Disposition'] = 'attachment; filename=routes_gps.json'
    return resp
...
@app.route('/api/import_route_data_gps', methods=['POST'])
def import_route_data_gps():
    data = request.get_json(force=True)
    settings = (data or {}).get('settings', {})
    node.route_data_gps = {
        'waypoints': data.get('waypoints', []),
    ...
    node.log_message(f"Imported GPS route data: {len(node.route_data_gps.get('waypoints', []))} waypoints, {len(node.route_data_gps.get('routes', {}))} routes.")
    return jsonify({'ok': True})
